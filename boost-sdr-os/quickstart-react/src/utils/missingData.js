...
//   the last 90 days (their SDR / Company / Source feed Scoreboard credit).
...
// Date, SDR on opportunities.

import { LEAD_COLS } from '../api/monday';

export const MISSING_BOARDS = { LEAD: 'lead', OPP: 'opportunity' };
...
const F = {
  // Leads
  leadSdr:        { id: LEAD_COLS.SDR,        label: 'SDR',                 type: 'multiple-person' },
...
export function buildMissingList({ leads, opportunities }, now = Date.now()) {
  const out = [];
