...
function StatBox({ label, value, sub }) {
  return (
    <div className="text-center">
      <div className="font-display font-semibold text-[22px] text-ink leading-none">{value}</div>
      {sub && <div className="text-[10px] text-emerald font-semibold mt-0.5">{sub}</div>}
  ...
  return (
    <div className="border border-line rounded-xl p-3.5 bg-canvas">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-[12px] font-heading font-bold text-ink">
          {mins != null ? `${mins}m` : '—'}
        </span>
      </div>
      {url ? (
        <a
          ...
          target="_blank"
          rel="noreferrer"
          className="inline-flex items-center gap-1.5 text-[12.5px] font-semibold text-navy hover:text-navy-700 transition-colors"
        >
  ...
  return (
    <>
      ...
      <div className="fixed inset-y-0 right-0 w-[400px] bg-card border-l border-line shadow-2xl z-50 flex flex-col overflow-hidden">
        ...
        {/* Header */}
        <div className="flex items-center gap-3 px-5 py-4 border-b border-line bg-sunken flex-shrink-0">
          {rep.photoThumb ? (
            <img src={rep.photoThumb} alt={rep.name} className="w-10 h-10 rounded-full object-cover flex-shrink-0" />
          ) : (
            <div className="w-10 h-10 rounded-full bg-navy text-white grid place-items-center font-heading font-bold text-[15px] flex-shrink-0">
              {initials}
            </div>
          )}
          <div className="flex-1 min-w-0">
            <div className="font-heading font-bold text-[16px]">{rep.name}</div>
        ...
        <div className="flex-1 overflow-y-auto">
          ...
          <div className="px-5 py-5 border-b border-line">
            <p className="text-[11px] font-semibold tracking-[.08em] uppercase text-muted mb-4">Call summary</p>
            ...
            <div className="mt-4 pt-4 border-t border-line text-center">
              ...
              <div className="font-display font-semibold text-[24px] text-ink">
                {avgMins > 0 ? `${avgMins}m` : '—'}
              </div>
            </div>
          </div>
          ...
          {/* Duration breakdown */}
          <div className="px-5 py-5 border-b border-line">
            <p className="text-[11px] font-semibold tracking-[.08em] uppercase text-muted mb-4">Duration breakdown</p>
            ...
            <DurationBar label="> 3 min (meaningful)"       count={over3.length}     total={totalDials} color="bg-emerald" />
          </div>

          {/* Sample recordings */}
          <div className="px-5 py-5">
            <p className="text-[11px] font-semibold tracking-[.08em] uppercase text-muted mb-1">Sample recordings</p>
