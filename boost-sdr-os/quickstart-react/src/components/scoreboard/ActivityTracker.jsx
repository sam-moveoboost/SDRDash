...
function MeetingStat({ label, value, loading, onClick }) {
  return (
    <button
      onClick={onClick}
      disabled={loading}
      className="flex-1 text-left rounded-xl border border-line bg-canvas/60 hover:border-teal px-3 py-2 transition-colors"
    >
      <div className={`font-display font-bold text-[20px] leading-none ${value > 0 ? 'text-mint-deep' : 'text-muted'}`}>
        {loading ? '—' : value}
      </div>
